...
const countryNames: Record<string, string> = {
  PH: "Philippines", US: "United States", BE: "Belgium", TH: "Thailand", GB: "United Kingdom", CA: "Canada",
  AU: "Australia", DE: "Germany", FR: "France", JP: "Japan", KR: "South Korea", CN: "China",
  IN: "India", SG: "Singapore", MY: "Malaysia", ID: "Indonesia", VN: "Vietnam", BR: "Brazil",
  MX: "Mexico", ES: "Spain", IT: "Italy", NL: "Netherlands", SE: "Sweden", NO: "Norway",
  DK: "Denmark", FI: "Finland", PL: "Poland", RU: "Russia", TR: "Turkey", SA: "Saudi Arabia",
  AE: "United Arab Emirates", IL: "Israel", EG: "Egypt", ZA: "South Africa", NG: "Nigeria", KE: "Kenya",
  AR: "Argentina", CL: "Chile", CO: "Colombia", PE: "Peru", NZ: "New Zealand", PT: "Portugal",
  CH: "Switzerland", AT: "Austria", IE: "Ireland", CZ: "Czech Republic", HU: "Hungary", RO: "Romania",
};
...
export default function UmamiSection({ umami, loading }: UmamiSectionProps) {
  const demoPageviews = [
    { x: "2025-01", y: 1500 }, { x: "2025-02", y: 1700 }, { x: "2025-03", y: 2000 },
    { x: "2025-04", y: 2500 }, { x: "2025-05", y: 2200 }, { x: "2025-06", y: 1800 },
    { x: "2025-07", y: 1600 }, { x: "2025-08", y: 1400 }, { x: "2025-09", y: 1200 },
    { x: "2025-10", y: 1100 },
  ];

  const demoSessions = [
    { x: "2025-01", y: 100 }, { x: "2025-02", y: 120 }, { x: "2025-03", y: 150 },
    { x: "2025-04", y: 180 }, { x: "2025-05", y: 160 }, { x: "2025-06", y: 140 },
    { x: "2025-07", y: 130 }, { x: "2025-08", y: 120 }, { x: "2025-09", y: 110 },
    { x: "2025-10", y: 100 },
  ];
  ...
  const months = pageviewsDataToUse.map((d: any) => monthNames[parseInt(d.x.split("-")[1], 10) - 1]);
...
const chartOptions = {
  ...
  series: [
    ...
    },
    {
      ...
      type: "bar",
      stack: "total",
      data: pageviewsDataToUse.map((d: any, i: number) => d.y - sessionsDataToUse[i].y),
  ...
  return (
    <div>
      <CardAnimationWrapper index={18} className="flex items-center gap-2 mb-1">
        ...
        <motion.h2 variants={childVariants} className="text-xl font-semibold">Umami</motion.h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6 auto-rows-fr">
        {loading
          ? Array.from({ length: 6 }).map((_, index) => (
              <DashboardCardSkeleton key={index} className="w-full h-full flex flex-col" />
            ))
          : (
            <>
              ...
              <CardAnimationWrapper index={25}>
                <DashboardCard
                  ...
                  value={formatDuration(umami?.websiteStats?.avgDuration?.value || 0)}
                  description=""
                  className="w-full h-full flex flex-col"
                />
              </CardAnimationWrapper>
      ...
      {!loading && pageviewsDataToUse.length > 0 && sessionsDataToUse.length > 0 && (
        <CardAnimationWrapper index={27}>
          <div className="p-4 rounded-lg shadow-md">
            <ReactECharts option={chartOptions} style={{ height: 400 }} />
            {isDemo && (
              <p className="text-center text-yellow-500 text-sm mt-2">This is a demo chart only.</p>
            )}
          </div>
        </CardAnimationWrapper>
      )}
      ...
      <CardAnimationWrapper index={17}>
        <motion.hr variants={childVariants} className="border-t-2 border-yellow-400 mb-4 mt-4" />
      </CardAnimationWrapper>
    </div>
  );
}